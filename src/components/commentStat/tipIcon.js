...
const TipIcon = (props) => {
    const {
        parent: {
            commentId, commenter, sidechainId, transactionId
        },
        user, apis, data
    } = props;
        ...
        <Space direction="horizontal" size={0}>
            {
                checked
                    ? (
                        <DollarCircleFilled
                            onClick={() => toggle()}
                            className="comment-stat-icon-tip comment-stat-icon-checked"
                            title={`Tip + fee: ${constants.fees[TransactionTypes.POSTTIP] / constants.fixedPoint} D.`} />
                    )
                    : (
                        <DollarOutlined
                            onClick={() => toggle()}
                            className="comment-stat-icon-tip comment-stat-icon"
                            title={`Tip + fee: ${constants.fees[TransactionTypes.POSTTIP] / constants.fixedPoint} D.`} />
                    )
            }
            <CoinNumber precision={2} className="comment-state-count" value={amount * constants.fixedPoint} />
            <Popover
                placement="top"
                open={showPopOver}
                title="Send tip"
